...
Current test build: **1.1.0-b**. Use `SnappySnap-Setup-1.1.0-b-x64.exe` on Windows 11 x64. Exit SnappySnap from its tray menu before installing. The offline per-user installer includes the required runtime; existing settings and captures are retained. The tester executable is not Authenticode-signed.
