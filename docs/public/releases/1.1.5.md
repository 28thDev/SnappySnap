# SnappySnap 1.1.5 — release candidate

This installer is for owner testing before publication. It supersedes 1.1.4 and is not a stable GitHub release.
...
Use `SnappySnap-Setup-1.1.5-x64.exe` on Windows 11 x64. Exit SnappySnap from its tray menu before installing. The offline per-user installer includes the required runtime; existing settings and captures are retained. The tester executable is not Authenticode-signed.
